...
def compile_one(host_path, strip_root=None, verbose=False):
    ...
    if strip_root is not None:
        ...
        runtime_path = os.path.join(
                "/", os.path.relpath(host_path, strip_root)
        )
    else:
    ...
    py_compile.compile(
            host_path, cfile=host_path + "c", dfile=runtime_path, doraise=True
    )
...
def main():
    ...
    parser.add_argument(
            "--verbose", action="store_true", help="Print compiled files"
    )
    ...
    try:
        for d in args.dirs:
            ...
            for parent, _, files in os.walk(d):
                for f in files:
                    compile_one(
                            os.path.join(parent, f), args.strip_root,
                            args.verbose
                    )
